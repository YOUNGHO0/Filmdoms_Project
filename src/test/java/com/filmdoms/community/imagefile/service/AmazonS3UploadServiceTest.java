...
class AmazonS3UploadServiceTest {
    ...
    public void 이미지를_업로드하면_DB에_이미지_정보가_저장됨() {
        ...
        Assertions.assertThat(savedImageFiles.size()).isEqualTo(5); //실제 이미지가 저장되는지 확인
        Assertions.assertThat(responseDto.getImageIds().size()).isEqualTo(5); //응답 DTO가 잘 생성되는지 확인
    }
}